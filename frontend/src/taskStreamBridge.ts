 ...
 *
 * <p>Multiple subscribers per taskId are supported through a small
 ...
 * filters events by taskId so callers only see what they asked for.
 */
type Subscription = {
...
export function registerTaskStreamIpc(getMainWindow: () => BrowserWindow | null): void {
  ipcMain.handle('tasks:stream:start', async (_e, id: unknown) => {
  ...
  ipcMain.handle('tasks:stream:stop', async (_e, id: unknown) => {
    if (typeof id !== 'string' || id.trim().length === 0) {
      return;
    }
  ...
  try {
    ...
    while (true) {
      ...
      while (nl !== -1) {
        ...
        nl = carry.indexOf('\n');
        if (line.length === 0) {
          dispatch(buf, taskId, getMainWindow);
          continue;
        }
        if (line.startsWith(':')) {
    ...
    if (win && !win.isDestroyed()) {
      win.webContents.send('tasks:stream:close', { taskId, reason });
    }
...
function dispatch(
  ...
  const win = getMainWindow();
  if (win && !win.isDestroyed()) {
    const ev: TaskStreamEvent = { name, data };
    win.webContents.send('tasks:stream:event', { taskId, event: ev });
  }
}
