...
import com.bytequay.app.service.review.TaskReviewRoundSnapshotRuntime.Status;
...
/** Delivers a later-round Task snapshot to its exact Task command owner. */
@Component
public final class TaskReviewRoundSnapshotResultDeliveryPort
        implements ExecutionPorts.ResultDeliveryPort
{
    ...
    public TaskReviewRoundSnapshotResultDeliveryPort(
            TaskReviewRoundSnapshotRuntime operations,
            ObjectProvider<InvestigationReviewService> reviews,
            TaskCommandExecutor commands, ObjectMapper json)
    {
            ...
            DispatchTicket.DispatchResult rawResult)
    {
        ...
        if (owner.kind() != TASK
                || !TaskReviewRoundSnapshotOperationHandler.CALLBACK_ROUTE.equals(
                        owner.callbackRoute())
        ...
        if (!Objects.equals(rawResult.payloadJson(), rawResult.evidenceJson())) {
            throw new IllegalArgumentException(
                    "Task review round snapshot payload and evidence differ");
        }
        ...
        if (!subject.current() || !result.subjectCurrent()) {
            operations.finishTerminal(subject.operationId(), Status.SUPERSEDED,
                    evidence, "Task code changed before snapshot delivery");
            return receipt(SUPERSEDED, evidence);
        }
        commands.executeVoid(subject.taskId(), () ->
                reviews.getObject().acceptTaskReviewRoundSnapshot(subject, result));
        return receipt(ACCEPTED, evidence);
    }
    ...
    private SnapshotResult decode(String value)
    {
        try {
            ...
            throw new IllegalArgumentException(
                    "Task review round snapshot returned invalid evidence", e);
        }
    }
            ...
            ExecutionSubject subject, DispatchTicket.DispatchResult result)
    {
        ...
        if (!exact) {
            throw new IllegalStateException(
                    "Task review round terminal replay differs from receipt");
        }
    }

    private static void requireOwner(
            DispatchTicket.OwnerReference owner,
            DispatchTicket.OperationFence fence, ExecutionSubject subject)
    {
        if (!subject.taskId().equals(owner.id())
            ...
            throw new IllegalArgumentException(
                    "Task review round snapshot delivery differs from its owner");
        }
    }
            ...
            ExecutionSubject subject, SnapshotResult result)
    {
        if (!subject.operationId().equals(result.operationId())
            ...
            throw new IllegalArgumentException(
                    "Task review round snapshot result differs from its subject");
        }
    }
