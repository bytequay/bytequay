...
type Props = {
  ...
  /** Swap to another thread's detail page from the rail's Recent list. */
 ...
 * {@code docs/mockups/design/threads/thread-detail-terminal.png}.
 *
 * <p>SSE through Electron is deferred; we poll on a status-aware
 * cadence (1s while RUNNING, 5s otherwise, off when terminal).
 */
/** Renderer for the thread window body. Two modes — the structured
 ...
 *  thread-window header bar. Diff (working-tree changes) opens
 *  side-by-side via the strip above the reply input, not a tab. */
type DetailView = 'conversation' | 'terminal';
const VIEW_STORAGE_KEY = 'bytequay.threads.detailView';
...
const DIFF_OPEN_STORAGE_KEY = 'bytequay.threads.detailDiffOpen';
...
const DIFF_SPLIT_FRAC_KEY = 'bytequay.threads.detailDiffSplitFrac';
...
const THEME_STORAGE_KEY = 'bytequay.threads.terminalTheme';
...
const SIDEBAR_COLLAPSED_STORAGE_KEY = 'bytequay.threads.detailSidebarCollapsed';
 ...
 *  threads-checkpoints-and-context.md for the parser change that will
...
export default function ThreadDetailPage({
  ...
  // but the detail view replaces the rail with ThreadWindowSidebar,
  ...
  const [thread, setTask] = useState<ThreadDto | null>(null);
  const [messages, setMessages] = useState<ThreadMessageDto[]>([]);
  const [turns, setTurns] = useState<ThreadTurnDto[]>([]);
  const [turnEvents, setTurnEvents] = useState<ThreadTurnEventDto[]>([]);
  const [files, setFiles] = useState<ThreadFileDto[]>([]);
  const [allTasks, setAllTasks] = useState<ThreadDto[]>([]);
  ...
  // still here. Keyed by threadId so per-thread drafts stay separate.
  const [draft, setDraft] = usePersistentDraft(`reply:${threadId}`);
  ...
  const refresh = useCallback(async () => {
    try {
      const [t, m, ts, tes, fs, list, gs, ms, wrs] = await Promise.all([
        ...
        window.bridge.getTaskFiles(threadId).catch(() => [] as ThreadFileDto[]),
  ...
  }, [thread, resolveTaskRepo, onOpenPr]);

  /** Add or remove this thread from a group. Mirrors the toggle on the
   *  ThreadsPage so the affordance feels the same wherever you find it.
  ...
  /** Group IDs the current thread belongs to. */
  const currentGroupIds = useMemo(
    () => memberships.filter(m => m.threadId === threadId).map(m => m.groupId),
  ...
  const onRename = useCallback(async (nextTitle: string) => {
    ...
    try {
      const updated = await window.bridge.renameTask(threadId, trimmed);
      setTask(updated);
    }
    catch (e) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  useEffect(() => {
    const interval = pollInterval(thread?.status, hasActiveTurn);
    if (!interval) return;
    const id = setInterval(() => { void refresh(); }, interval);
    return () => clearInterval(id);
  }, [thread?.status, hasActiveTurn, refresh]);
  ...
  // we don't open a second SSE stream per thread — one subscriber,
  // multiple consumers. Null when the panel isn't mounted (e.g.
  // empty thread or terminal view).
  ...
  // Only opens while the thread is still live; terminal-status
  // sessions don't emit anything new. Re-runs only when the thread
  ...
  const stage = useMemo(() => deriveStage(messages, thread?.status), [messages, thread?.status]);
  ...
  const onStop = useCallback(async () => {
    try {
      await window.bridge.stopTask(threadId);
      await refresh();
    }
    catch (e) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  // "Rendered more hooks than during the previous render" when thread
  ...
  const diffState = useTaskDiffState(threadId);
 ...
 *  the list. */
function useMemoChangeStats(files: ThreadFileDto[]) {
  return useMemo(() => {
    let added = 0;
    let removed = 0;
...
/** Click-to-edit thread title. Renders as a plain heading by default;
 ...
 *    the thread is RUNNING, surfacing the streaming-in indicator and
 ...
 *
 * Follows {@code docs/mockups/design/threads/thread-detail.png}. Reuses
...
function StructuredView({
  ...
  onDelete: () => void;
  canDelete: boolean;
  /** Flip an ERRORED thread back to IDLE so the user can keep typing.
  ...
  /** Thread id used by the floating ConvIndex panel to fetch its
    ...
    <div style={structuredWrapStyle}>
      <div style={historyZoneStyle}>
        <div style={zoneHeaderStyle}>
          <div style={threadTitleBadgeStyle}>
            {/* Same shape the terminal toolbar uses: a 20-word JS
                ...
                (the bug in docs/mockups/issue/threads/name.png). */}
            <EditableTitle
              title={thread.title}
              onRename={onRename}
              maxDisplayWords={20}
        ...
        {/* Floating right-edge index panel — anchored to the
            ...
            automatically when the thread has no user prompts yet. */}
        <ConvIndex
          threadId={threadId}
...
/**
 * Left-rail sidebar for the thread detail page. Replaces the global
 * ThreadsLeftRail while a thread is open. Layout follows
 * docs/mockups/design/threads/thread-detail-tabs.png:
 *
 *  • Back button (returns to the thread list)
 ...
 *    captured in followups/threads-checkpoints-and-context.md
 */
function ThreadWindowSidebar({
  thread, modelName, messages, turns, turnEvents, files, liveUsage,
  ...
  const tokensInDisplay = (thread.tokensIn ?? 0) + (liveUsage?.tokensIn ?? 0);
  const tokensOutDisplay = (thread.tokensOut ?? 0) + (liveUsage?.tokensOut ?? 0);
  return (
    <aside style={twSidebarStyle}>
      ...
      {/* Section labelled "Lifetime" — the metrics here are
          thread-lifetime totals, not today-only. The old "Today" label
      ...
      <SidebarSection label="Lifetime">
        <div style={metricListStyle}>
          <Metric label="Runtime" value={formatRuntime(thread)} live={thread.status === 'RUNNING'} />
          <Metric label="Cost" value={formatCost(thread.costUsdMilli)} />
          <Metric
            label="Tokens in"
          ...
          <Metric label="Model" value={threadModelLabel(modelName)} mono />
        </div>
      </SidebarSection>
      ...
      {/* Session ID = our internal thread UUID (the route key,
          /threads/:id). The Claude Code CLI's own session id stays
          on the Thread row (`agentSessionId`) for backend-only use —
      ...
      <SidebarSection label="Session">
        <div style={metricListStyle}>
          <Metric label="Session ID" value={thread.id} mono wrap />
      ...
      <SidebarSection label="Checkpoints">
        <CheckpointsSection threadId={thread.id} sseRef={checkpointsSseRef} />
      </SidebarSection>
    </aside>
  );
}
 ...
 *  toolbar in terminal mode). The top-level ThreadWindowHeader used to
 ...
 *  look broken. See followups/threads-checkpoints-and-context.md. */
...
function computeContextUsage(messages: ThreadMessageDto[], model: string | null) {
...
function TerminalWrap({
  thread, modelName, messages, pendingPermission, onDecide, stage,
  ...
  return (
    <div style={terminalWrapStyle}>
      <div style={termToolbarStyle}>
        ...
        <div style={threadTitleBadgeTermStyle}>
          <EditableTitle
            title={thread.title}
            onRename={onRename}
            maxDisplayWords={20}
      ...
      <TerminalStatusBar thread={thread} stage={stage} liveUsage={liveUsage} />
...
function TerminalStatusBar({
  ...
  const isRunning = thread.status === 'RUNNING';
  const tokensTotal = (thread.tokensIn ?? 0) + (thread.tokensOut ?? 0)
  ...
  return (
    <div style={termStatusBarStyle}>
      ...
      <span style={termStatGroupRightStyle}>
        <span style={termStatStyle}>runtime <strong style={termStatStrongStyle}>{formatRuntime(thread)}</strong></span>
        <span style={termStatStyle}>cost <strong style={termStatStrongStyle}>{formatCost(thread.costUsdMilli)}</strong></span>
        <span style={termStatStyle} title="Total input and output tokens">
          <strong style={termStatStrongStyle}>{threadTokenLabel(tokensTotal)}</strong>
        </span>
        {stage.toolName && (
...
function deriveToolUsage(messages: ThreadMessageDto[]): ToolUsage {
...
function SchedulerEventHistory({ events, turns }: { events: ThreadTurnEventDto[]; turns: ThreadTurnDto[] }) {
...
function StatusPill({ status }: { status: ThreadStatusDto }) {
...
function summarizeTurnState(turns: ThreadTurnDto[], threadStatus: ThreadStatusDto | undefined): SchedulerSummary {
  const running = turns.find(t => t.status === 'RUNNING') ?? null;
  const queued = turns.filter(t => t.status === 'QUEUED');
  // Once a thread is alive again (IDLE / AWAITING / RUNNING / PENDING)
  ...
  // the user into thinking the thread is still dead. Anchor the state
  ...
  // No in-flight work. Reflect the thread's own status when it's still
  // alive (so resumed threads read "ready"), and fall back to the most
  // recent finished turn only for terminal threads.
  if (liveTaskStatus) {
    return {
      state: 'ready',
...
function deriveStage(messages: ThreadMessageDto[], status: ThreadStatusDto | undefined): Stage {
...
function resolvedModelName(threadModel: string | null, messages: ThreadMessageDto[]): string {
  const stored = (threadModel ?? '').trim();
  if (stored !== '') {
    return stored;
  }
  for (let i = messages.length - 1; i >= 0; i--) {
...
function pollInterval(status: ThreadStatusDto | undefined, hasActiveTurn: boolean): number {
...
function formatRuntime(thread: ThreadDto): string {
  ...
  const end = thread.endedAt ? new Date(thread.endedAt).getTime() : Date.now();
  ...
  // minute rollovers — the page polls /threads/{id} every 1s while
...
function schedulerEventMeta(event: ThreadTurnEventDto['event']): { label: string; hint: string; color: string } {
  switch (event) {
    case 'TURN_QUEUED':
      return {
        label: 'Submitted',
    ...
    case 'WAITING_FOR_CAPACITY':
      return {
        label: 'Queued',
        hint: 'Still waiting because the lane or this thread is busy.',
        color: '#b45309',
      };
    case 'TURN_STARTED':
      return {
...
function queuedDurationLabel(event: ThreadTurnEventDto, turn: ThreadTurnDto | null): string {
...
const layoutStyle: React.CSSProperties = {
  ...
  // — thread window → body → terminal/structured pane → ConversationPane
...
// ────────────────────────────────────────────────────────────────────
// New detail-page shell — sidebar + thread window + bottom tabs.
// Follows docs/mockups/design/threads/thread-detail-tabs.png.
// ────────────────────────────────────────────────────────────────────
...
const threadWindowStyle: React.CSSProperties = {
  flex: 1, minWidth: 0,
  display: 'flex', flexDirection: 'column',
  background: 'var(--bg-card)',
  borderLeft: '1px solid var(--border)',
};
const threadWindowBodyStyle: React.CSSProperties = {
  flex: 1, minHeight: 0,
  display: 'flex', gap: 12,
  padding: 12,
};
...
// Left sidebar (ThreadWindowSidebar) ──────────────────────────────────
...
// docs/mockups/v2/threads/_src/thread-detail-tabs.html. Each rail row is
...
// Amber tint when the thread has touched files (lifetime). Replaces
...
const threadTitleBadgeStyle: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center',
  padding: '2px 8px',
  background: 'var(--bg-elevated)',
  border: '1px solid var(--border)',
  borderRadius: 6,
  // Mirror `threadTitleBadgeTermStyle` exactly: `width: max-content`
  // sizes the badge to the full title length so the flex container
  // can't squeeze it back to "let's …" (see
  // docs/mockups/issue/threads/name.png). The caller pairs this with
  // `maxDisplayWords` on EditableTitle to JS-truncate essay-long
  // titles before they get to the CSS layer.
  flexShrink: 0,
  width: 'max-content',
};
const threadTitleBadgeTermStyle: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center',
  padding: '1px 8px',
  ...
  // docs/mockups/issue/threads/name.png — that was the bug being fixed).
  flexShrink: 0,
  width: 'max-content',
};
...
// would occupy when the thread is live, so its sizing intentionally
