 ...
 * the message-range metadata before calling {@code ThreadCheckpointStore}.
...
public class CheckpointSummariser
{
    ...
    private static final String OVERALL_SYSTEM_PROMPT = """
            You are summarising a thread end-to-end given its per-segment summaries. Produce \
            a single rollup of 200–350 words in Markdown: start with 2–3 bullet titles, \
            then 1–2 paragraphs that capture the through-line of the thread. Treat earlier \
     ...
     */
    public CheckpointSummaryResult summariseSegment(String threadId, long firstSeq, long lastSeq)
    {
        requireNonNull(threadId, "threadId is null");
        List<ThreadMessage> msgs = store.listMessagesBetween(threadId, firstSeq, lastSeq);
        if (msgs.isEmpty()) {
     ...
     */
    public CheckpointSummaryResult refreshOverall(String threadId, List<ThreadCheckpoint> segments)
    {
        requireNonNull(threadId, "threadId is null");
        requireNonNull(segments, "segments is null");
        if (segments.isEmpty()) {
            throw new IllegalStateException(
                    "refreshOverall called with no segments for thread " + threadId);
        }
        StringBuilder sb = new StringBuilder();
        for (ThreadCheckpoint cp : segments) {
     ...
     */
    String renderForSummary(List<ThreadMessage> msgs)
    {
