 ...
 * from {@code task_messages} on demand — the doc explicitly calls
 ...
 *       with derived index entries plus the task-wide user-prompt
...
public class ConvIndexService
{
    ...
    public ConvIndexService(TaskStore store, ObjectMapper mapper)
    {
     ...
     *  count is task-wide so the header can show progress beyond
     *  the current window. */
    public ConvIndexPage initial(String taskId, int limit)
    {
        requireNonNull(taskId, "taskId is null");
        int capped = capLimit(limit);
        List<TaskMessage> window = store.listRecentMessages(taskId, capped);
        long total = store.countUserMessages(taskId);
        return buildPage(taskId, total, window);
    }
    ...
    public ConvIndexPage backfill(String taskId, long beforeSeq, int limit)
    {
        requireNonNull(taskId, "taskId is null");
        int capped = capLimit(limit);
        List<TaskMessage> window = store.listMessagesBefore(taskId, beforeSeq, capped);
        long total = store.countUserMessages(taskId);
        return buildPage(taskId, total, window);
    }

    private ConvIndexPage buildPage(String taskId, long total, List<TaskMessage> window)
    {
        ImmutableList.Builder<ConvIndexEntry> entries = ImmutableList.builder();
        for (TaskMessage m : window) {
            if (isUserPrompt(m)) {
        ...
        // task's seq=1 row. A brand-new task that returns 0 rows
