...
class TestAutoGatingStep
{
    ...
    private final AutoGatingStep step = new AutoGatingStep(registry, responses);

    @Test
    void autoToolIsAllowedWithoutAPrompt()
    {
    ...
    @Test
    void gatedToolFallsThroughToLaterSteps()
    {
        stub("open_pr", Gating.GATED);
        assertThat(step.apply(ctx("mcp__bytequay__open_pr")))
                .isInstanceOf(ApprovalStepResult.Continue.class);
    }

    @Test
    void parkedToolFallsThrough()
    {
        stub("push", Gating.PARKED);
        assertThat(step.apply(ctx("mcp__bytequay__push")))
                .isInstanceOf(ApprovalStepResult.Continue.class);
    }
