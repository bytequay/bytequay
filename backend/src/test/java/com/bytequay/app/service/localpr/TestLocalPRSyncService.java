...
class TestLocalPRSyncService
{
    ...
    private final LocalPRPublishService localPrPublish = mock(LocalPRPublishService.class);
    private final LocalPRSyncService service =
            new LocalPRSyncService(localPr, taskStore, git, brainReview, pullRequests, localPrPublish);
            ...
            throws Exception
    {
        ...
        when(localPr.createForTask(any(), any(), any(), any(), any())).thenReturn(pushedPr());
        when(localPr.commits("pr1")).thenReturn(List.of());
        when(git.resolveCommitBase(any(), any())).thenReturn("main");
        when(git.listCommitsAhead(any(), any(), anyInt())).thenReturn(List.of());
        when(localPr.findById("pr1")).thenReturn(Optional.of(pushedPr()));
        ...
        when(localPr.hasRemoteEvent(eq("pr1"), anyLong())).thenReturn(false);

        service.syncFromTask("task1");

        verify(localPr).addRemoteComment("pr1", "@octocat", "Can you also handle nulls?", NOW, 5001L);
        verify(localPr).recordRemoteReview("pr1", "@reviewer1", "APPROVED", "Nice cleanup, LGTM.", NOW, 9001L);
    }
            ...
            throws Exception
    {
        ...
        when(pullRequests.getPullRequestDetail("acme/widget", 42)).thenReturn(detail);
        when(localPr.hasRemoteEvent("pr1", 5001L)).thenReturn(true);

        service.syncFromTask("task1");

        verify(localPr, never()).addRemoteComment(any(), any(), any(), any(), anyLong());
    }
    ...
    @Test
    void healsALocalPrRowStuckBehindAnAlreadyOpenRemotePr()
            throws Exception
    {
        ...
        // the remote PR (linkedPrRef), but the LocalPR row never advanced
        ...
        verify(localPrPublish).onPushedElsewhere(
                new LocalPrPushedEvent("task1", 32, "https://github.com/acme/widget/pull/32"));
    }
            ...
            throws Exception
    {
        ...
        verify(localPrPublish, never()).onPushedElsewhere(any());
    }
