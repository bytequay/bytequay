...
public class DevReportController
{
    ...
    @GetMapping("/api/tasks/{taskId}/dev-report")
    public DevReport devReport(@PathVariable String taskId)
    {
        return devReports.find(taskId)
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatusCode.valueOf(404), "no dev report for task: " + taskId));
    }
}
