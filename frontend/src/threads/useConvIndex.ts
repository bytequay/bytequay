...
/**
 * Reads the conversation index for a thread and keeps it in sync.
 *
 ...
 */
export function useConvIndex(threadId: string): ConvIndexState & {
  ...
  // Initial load on thread switch. Resets state so a leftover index
  ...
  const loadOlder = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const page = await window.bridge.getTaskIndex(threadIdRef.current, {
        cursor,
  ...
  const onUpstreamEvent = useCallback((eventName: string) => {
    if (eventName === 'UserMessage' || eventName === 'TurnDone') {
      void fetchInitial(threadIdRef.current);
    }
  }, [fetchInitial]);
