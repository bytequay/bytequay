...
public interface PullRequestRepository
{
    ...
    /**
     * Looks up the PR's merge-queue facts via GraphQL, consulting the base
     * branch's active REST rules when GraphQL exposes neither a queue nor an
     * entry. Returns both whether the base branch has a merge queue configured
     * (it's possible to add this PR to the queue) and the PR's current entry
     * state ({@code QUEUED}, {@code MERGEABLE}, {@code UNMERGEABLE}, etc.), or
    ...
    /**
     * Merge-queue facts for a single PR, sourced from GraphQL and active
     * branch rules.
     *
            ...
            String detail)
    {
        public ActionsJobLogCapture
        {
            ...
            if (status == ActionsJobLogStatus.COMPLETE) {
                ...
                if (rawText.isEmpty() || rawByteLength < 1
            ...
            else if (rawText != null || rawByteLength != null
                    || sha256Digest != null || detail == null
                    || detail.isBlank()) {
                throw new IllegalArgumentException(
        ...
        public static ActionsJobLogCapture complete(
                long jobId, String rawText, long rawByteLength,
                String sha256Digest)
        {
            return new ActionsJobLogCapture(
                    jobId, ActionsJobLogStatus.COMPLETE, rawText,
                    rawByteLength, sha256Digest, null);
        }

        public static ActionsJobLogCapture unavailable(long jobId, String detail)
        {
            return new ActionsJobLogCapture(
                    jobId, ActionsJobLogStatus.UNAVAILABLE,
        ...
        {
            return new ActionsJobLogCapture(
                    jobId, ActionsJobLogStatus.INCOMPLETE,
                    null, null, null, detail);
        }
    }

    /**
     * Downloads one already-bound Actions job directly. Provider transport
     * failures remain exceptions so the caller can retry the read-only
     * operation; unavailable or malformed evidence is returned explicitly.
     */
    default ActionsJobLogCapture fetchActionsJobLogStrict(
            String pat, RepoRef repo, long jobId)
    {
        throw new UnsupportedOperationException(
                "fetchActionsJobLogStrict not implemented");
    }
            ...
            boolean complete)
    {
        ...
        {
            failureAnnotations = List.copyOf(requireNonNull(
                    failureAnnotations, "failureAnnotations is null"));
        }
    }
    ...
    /**
     * Fetches every annotation page for an exact check run. Unlike the UI
     * helper above, this retains generic failure annotations so the Remote CI
     * proof builder can reject generic-only evidence instead of mistaking it
     * for a specific failure fingerprint.
     */
    default CheckRunAnnotationEvidence fetchCheckRunAnnotationsStrict(
            ...
            int expectedAnnotationCount)
    {
        List<CheckRunAnnotation> annotations = fetchCheckRunAnnotations(
                pat, repo, checkRunId);
        return new CheckRunAnnotationEvidence(
                annotations, annotations.size(), expectedAnnotationCount,
