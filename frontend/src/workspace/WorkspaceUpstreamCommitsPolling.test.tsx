...
const running: UpstreamCherryPickJobDto = {
  jobId: 'job-1', status: 'RUNNING', resultBranch: 'trino-482', requestedCount: 1,
...
const repository: WorkspaceRepositoryDto = {
  owner: 'acme', repo: 'cork', fullName: 'acme/cork', defaultBaseBranch: 'main',
  local: {
    owner: 'acme', repo: 'cork', localClonePath: '/tmp/cork', state: 'CLEAN',
...
describe('UpstreamCherryPicker durable polling', () => {
  ...
  it('allows a new range after rediscovering a historical failed job', async () => {
    ...
    expect(screen.getByDisplayValue('trino-482')).toBeTruthy();
  });
});
