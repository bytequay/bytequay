 ...
 * any path other than {@link LocalPRPublishService#push} itself — a push /
 * open_pr gate (auto-approved or not), or the ship/next tool flow. The
 * listener syncs the task's LocalPR row so its own panel doesn't keep
 * offering "ready to push" for a push that already happened elsewhere.
 *
 * @param taskId the task whose LocalPR row to sync
 * @param remotePrNumber the PR now open on the remote
 * @param remotePrUrl the PR's github.com URL
 */
public record LocalPrPushedEvent(String taskId, int remotePrNumber, String remotePrUrl)
{
}
