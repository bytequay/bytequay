...
/**
 * The three tiers in the human-readable id hierarchy and the marker
 * each one stamps into a generated id. The single source of truth for
 * the {@code "ws-"} / {@code ".t"} / {@code ".k"} literals — anyone
 * reading or producing ids should reach for {@link #marker()} rather
 ...
 *                 e.g.  ws-bytequay
 *   Thread      {workspaceId}{THREAD.marker()}{ymd}-{seq}-{rand2}
...
public enum IdTier
{
    ...
    /** Separator between a workspace slug and the thread's date + seq + rand. */
    THREAD(".t"),
