...
/** Optional extra payload for a permission reply. When present, the
 *  backend records the per-call decision and then grants an
 *  auto-approval budget for future calls of the same tool. */
...
/** Options for the "Allow next N" dropdown. The count is the number
 *  of *additional* auto-allows we grant beyond the current call; the
 *  current call itself is always allowed first. -1 is "always". */
...
function PermissionCard({ permission, onDecide }: {
  ...
  // Close the picker when clicking outside — native <select> would
  // handle this for free, but we want a custom popover for the look.
  useEffect(() => {
    if (!menuOpen) return;
    ...
    <article style={permissionCardStyle}>
      ...
      <div style={permissionActionsStyle}>
        ...
        <div ref={menuRef} style={{ position: 'relative', display: 'inline-flex' }}>
          ...
          >▾</button>
          {menuOpen && (
            <div role="menu" style={preApproveMenuStyle}>
              <div style={preApproveMenuHeaderStyle}>auto-approve</div>
              {PRE_APPROVE_CHOICES.map((c, i) => (
                <button
                  ...
                  style={{
                    ...preApproveMenuItemStyle,
                    ...(i === choiceIdx ? preApproveMenuItemSelectedStyle : null),
                  }}
                >{c.menu}</button>
              ))}
            </div>
...
const preApproveMenuStyle: React.CSSProperties = {
...
const preApproveMenuHeaderStyle: React.CSSProperties = {
  fontSize: 10, color: 'var(--text-4)',
  textTransform: 'uppercase', letterSpacing: 0.5,
  padding: '4px 8px 2px',
};
const preApproveMenuItemStyle: React.CSSProperties = {
  background: 'transparent', border: 'none',
  textAlign: 'left', padding: '6px 8px',
  borderRadius: 4, cursor: 'pointer',
  fontSize: 12, color: 'var(--text-1)',
};
const preApproveMenuItemSelectedStyle: React.CSSProperties = {
