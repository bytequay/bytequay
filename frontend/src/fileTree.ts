/*
 * Builds a directory tree from flat file paths, then compacts any chain of
 * single-child directories into one node (github's "Files changed" tab does
 * this — a long unique path like `src/main/java/com/foo` shows as one row
 * instead of six).
 *
 * The compaction rule: a directory merges with its only child iff that child
 * is itself a directory. A directory with one file child stays a directory
 * with that file underneath, so the file name is visible on its own row.
 */
