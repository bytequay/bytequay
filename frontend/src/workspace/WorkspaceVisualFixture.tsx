...
function InboxStudy() {
  const trinoReview = visualDashboardPrs.find(value => value.number === 29586)!;
  const remoteReview = {
    ...trinoReview,
    id: 'starburstdata/cork#4062',
    repo: 'starburstdata/cork',
    number: 4062,
  ...
  const merged = {
    ...remoteReview,
    id: 'apache/stargate#30948',
    repo: 'apache/stargate',
    number: 30948,
  };
  const bytequay = {
    ...remoteReview,
  ...
  const items: InboxItem[] = [
    {
      ...
      read: false,
      source: { kind: 'pr', pr: trinoReview },
    },
    {
      id: 'visual-review-remote',
      type: 'review',
      title: 'Review requested on #4062',
      sub: '[Cherry Pick] Skip Iceberg Glue column comment caching when content is invalid · starburstdata/cork',
    ...
    },
    {
      ...
      type: 'done',
      title: '#30948 merged',
      sub: 'Add starburst-credentials shared component',
  ...
  const handlers: InboxHandlers = {
    ...
    workspaceForRepo: (owner, repo) => {
      ...
      if (fullName === 'apache/stargate') {
        return { workspaceId: 'workspace-stargate', name: 'stargate' };
      }
      return null;
    },
    dismiss: () => {},
