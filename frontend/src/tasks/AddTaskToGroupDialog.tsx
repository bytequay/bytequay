...
type Props = {
  group: TaskGroupDto;
  /** Every task in the workspace. The dialog filters out tasks
   *  already in this group (and any not-yet-pinnable terminal
   *  ones) on its own. */
  allTasks: TaskDto[];
  /** Group IDs each task belongs to — used to hide tasks that are
   *  already members of this group. */
  groupIdsByTaskId: Map<string, string[]>;
  onClose: () => void;
  /** Caller routes to the create-task page with this group pre-
   *  selected. The dialog closes itself before invoking so the
   *  navigation isn't competing with a fading overlay. */
  onCreateNew: () => void;
  /** Add an existing task into the group. Returning a promise
   *  lets the dialog flip into a busy state while the membership
   *  POST is in flight. */
  onAddExisting: (taskId: string) => Promise<void>;
};

/**
 * Two-path picker shown when the user clicks "+ Add task" in the
 * group rail. The top row jumps to the create-task page; the list
 * below lets the user pull in a task that already exists. Single-
 * pick: tap a row, the task is added, the dialog closes.
 */
export default function AddTaskToGroupDialog({
  ...
  return (
    <div style={overlayStyle} onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Add a task to ${group.name}`}
      >
        <header style={headerStyle}>
          <h2 style={titleStyle}>Add a task to {group.name}</h2>
          <p style={subtitleStyle}>
            Start a new task, or pull an existing one in.
          </p>
        </header>
        ...
        >
          ...
          <span style={createLabelStyle}>
            <span style={createPrimaryStyle}>Create a new task</span>
            <span style={createHintStyle}>Opens the new-task form with this group pre-filled.</span>
          </span>
        </button>
        ...
        <div style={listStyle}>
          {eligible.length === 0 ? (
            <div style={emptyStyle}>
              No other tasks available — every task you have is already
              in this group.
            </div>
          ) : matched.length === 0 ? (
            <div style={emptyStyle}>
...
const STATUS_TONES: Record<TaskDto['status'], { fg: string; bg: string; border: string }> = {
