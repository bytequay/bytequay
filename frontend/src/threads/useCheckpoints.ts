...
export type CheckpointsState = {
   ...
   *  thread (e.g. "Anthropic API key not configured"). Null when the
...
/**
 * Reads the active checkpoints for a thread and keeps them in sync.
 *
 ...
 * lands in the rail.
 */
export function useCheckpoints(threadId: string): CheckpointsState & {
  ...
  // Track current thread in a ref so a slow response from the previous
  // thread can't paint over a new one after a thread switch.
  ...
  // Initial load on thread switch. Reset state so a leftover list from
  ...
  const generate = useCallback(async () => {
    ...
    setGenerateError(null);
    try {
      await window.bridge.generateTaskCheckpoint(threadIdRef.current);
      await refresh(threadIdRef.current);
    }
    catch (e) {
  ...
  const onUpstreamEvent = useCallback((eventName: string) => {
    if (eventName === 'TurnDone') {
      void refresh(threadIdRef.current);
    }
  }, [refresh]);
