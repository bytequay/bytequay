 ...
 * {@code docs/mockups/design/threads/thread-detail.png}:
 *
...
export function StructuredConversation({
  ...
  // Cap rendered history so a multi-hour thread with thousands of
...
function groupAndPair(messages: ThreadMessageDto[]): Event[] {
  const out: Event[] = [];
...
type Card =
  ...
  | { kind: 'assistant'; key: string; items: AssistantItem[]; modelName: string;
      tsIso: string; isStreaming: boolean; turn: ThreadMessageDto | null }
  | { kind: 'error'; key: string; message: ThreadMessageDto }
  | { kind: 'lifecycle'; key: string; message: ThreadMessageDto };
  ...
  return (
    <article style={assistantCardStyle}>
      <header style={cardHeaderStyle}>
        <span style={claudeAvatarStyle}>C</span>
        <span style={cardNameStyle}>Claude</span>
        <span style={modelBadgeStyle}>{threadModelLabel(modelName)}</span>
...
function ThinkingRow({ message }: { message: ThreadMessageDto }) {
...
function ProseRow({ message }: { message: ThreadMessageDto }) {
  const text = String(parseContent(message.contentJson).text ?? '');
  return (
    <div style={proseRowStyle}>
      <MarkdownProse text={text} variant="card" />
...
function AutoAllowRow({ message }: { message: ThreadMessageDto }) {
...
function ToolRow({ call, result }: { call: ThreadMessageDto; result: ThreadMessageDto | null }) {
...
function TurnFooter({ turn }: { turn: ThreadMessageDto }) {
...
function ErrorCard({ message }: { message: ThreadMessageDto }) {
