 ...
 * {@code docs/mockups/tasks-design.md} → "Two visual modes:
...
type Card =
  ...
  | { kind: 'assistantProse'; key: string; message: TaskMessageDto }
...
function buildCards(messages: TaskMessageDto[]): Card[] {
  const out: Card[] = [];
...
function UserBubble({ message, mode }: { message: TaskMessageDto; mode: TileConversationMode }) {
...
function AssistantProse({ message, mode }: { message: TaskMessageDto; mode: TileConversationMode }) {
  const text = (parseContent(message.contentJson).text as string) ?? '';
  if (mode === 'terminal') {
    return (
...
function ThinkingRow({ message, mode }: { message: TaskMessageDto; mode: TileConversationMode }) {
  const text = (parseContent(message.contentJson).text as string) ?? '';
...
function ErrorRow({ message, mode }: { message: TaskMessageDto; mode: TileConversationMode }) {
  const text = (parseContent(message.contentJson).text as string)
...
/** 3px inset bar colour per tool, taken from tasks-design.md
