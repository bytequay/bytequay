...
/** Executes the checks for one exact immutable Local code subject. */
public final class LocalValidationOperationHandler
        implements ExecutionPorts.OperationHandler
{
    public static final String OPERATION_KIND = "VALIDATE_LOCAL_DEVELOPMENT";
            ...
            throws Exception
    {
        ...
        ValidationResult result = new ValidationResult(
                1, context.validationOperationId(), context.operationId(),
                context.taskId(), context.taskEpoch(), context.stageId(),
                context.stageGeneration(), context.semanticAttempt(), current,
                current && failures.isEmpty(), List.copyOf(failures),
                after.fingerprint(), after.headSha(), after.baseSha(),
                startedAt.toEpochMilli(), clock.instant().toEpochMilli());
    ...
    private String write(Object value)
    {
        try {
            ...
            throw new IllegalStateException(
                    "Could not serialize Local validation result", e);
        }
    }
            ...
            long completedAtMs)
    {
        public ValidationResult
        {
            if (schemaVersion != 1 || taskEpoch < 1 || stageGeneration < 1
                ...
                throw new IllegalArgumentException(
                        "Local validation result identity is invalid");
            }
            requireNonNull(failures, "failures is null");
            failures = List.copyOf(failures);
            if (!subjectCurrent && passed || passed && !failures.isEmpty()) {
                throw new IllegalArgumentException(
                        "Local validation verdict is inconsistent");
            }
        }
    }
