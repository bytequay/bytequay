...
class TestCheckpointScheduler
{
    ...
    void thresholdCrossingGeneratesFirstSegmentAndOverall()
    {
        ...
        Optional<TaskCheckpoint> seg = ckps.findLastSegment("t1");
        ...
        Optional<TaskCheckpoint> overall = ckps.findActiveOverall("t1");
    ...
    void secondSegmentRangeStartsAfterPriorSegment()
    {
        ...
        List<TaskCheckpoint> active = ckps.listActive("t1");
    ...
    void manualGenerateSummarisesEvenBelowThreshold()
    {
        ...
        Optional<TaskCheckpoint> produced = scheduler.manualGenerate("t1");
    ...
    void manualGenerateReturnsEmptyWhenNothingNewSinceLastSegment()
    {
        InMemoryTaskStore tasks = new InMemoryTaskStore();
        InMemoryCheckpointStore ckps = new InMemoryCheckpointStore();
        tasks.seed("t1", msg(1, 5_000));
        // Pretend cp-1 already covers seq 1.
        ckps.saveSegment(new TaskCheckpoint(
    ...
    void overallRollupReceivesSegmentsInAscendingSeqOrder()
    {
        ...
        ArgumentCaptor<List<TaskCheckpoint>> rolledUp = ArgumentCaptor.forClass(List.class);
    ...
    private static CheckpointScheduler newScheduler(
            InMemoryTaskStore tasks, InMemoryCheckpointStore ckps, CheckpointSummariser summariser)
    {
        return new CheckpointScheduler(tasks, ckps, summariser, sameThreadExecutor(), THRESHOLD);
    }

    /**
     * Runs submitted tasks inline so onTurnDone's side effects are
    ...
    private static TaskMessage msg(long seq, long tokens)
    {
    ...
    private static TaskCheckpoint seg(
            String taskId, long seq, long firstMsg, long lastMsg, long tokens, Instant when)
    {
    ...
    /** Minimal TaskStore that only serves the message-side queries the
     *  scheduler hits. */
            ...
            implements TaskStore
    {
        ...
        {
            byTask.put(taskId, new ArrayList<>(List.of(msgs)));
        }

        void append(String taskId, TaskMessage... msgs)
        {
            byTask.computeIfAbsent(taskId, k -> new ArrayList<>()).addAll(List.of(msgs));
        }

        @Override
        public List<TaskMessage> listMessages(String taskId)
        {
            return List.copyOf(byTask.getOrDefault(taskId, List.of()));
        }
     ...
     *  impl enforces: one active Overall per task, monotonic seq for
    ...
    {
        private final Map<String, List<TaskCheckpoint>> byTask = new HashMap<>();
        ...
        public void saveSegment(TaskCheckpoint segment)
        {
            ...
            byTask.computeIfAbsent(segment.taskId(), k -> new ArrayList<>()).add(segment);
        }
        ...
        public void replaceOverall(String taskId, TaskCheckpoint next)
        {
            ...
            List<TaskCheckpoint> rows = byTask.computeIfAbsent(taskId, k -> new ArrayList<>());
            // Mark prior active Overall as superseded so listActive
            // filters it out, matching the SQLite store's behaviour.
            for (int i = 0; i < rows.size(); i++) {
                TaskCheckpoint cp = rows.get(i);
                if (cp.isOverall() && cp.supersededAt() == null) {
                    rows.set(i, new TaskCheckpoint(
                            cp.id(), cp.taskId(), cp.seq(), true,
        ...
        {
            List<TaskCheckpoint> all = byTask.getOrDefault(taskId, List.of());
            List<TaskCheckpoint> out = new ArrayList<>();
            for (TaskCheckpoint cp : all) {
                if (cp.supersededAt() == null) {
                    out.add(cp);
                }
            }
            out.sort(Comparator
                    .<TaskCheckpoint, Boolean>comparing(TaskCheckpoint::isOverall).reversed()
                    .thenComparing(Comparator.comparingLong(TaskCheckpoint::seq).reversed()));
        ...
        {
            for (List<TaskCheckpoint> rows : byTask.values()) {
                for (TaskCheckpoint cp : rows) {
                    if (cp.id().equals(id)) {
        ...
        {
            for (TaskCheckpoint cp : byTask.getOrDefault(taskId, List.of())) {
                if (cp.isOverall() && cp.supersededAt() == null) {
                    return Optional.of(cp);
                }
            }
        ...
        {
            return findLastSegment(taskId).map(cp -> cp.seq() + 1L).orElse(1L);
        }
