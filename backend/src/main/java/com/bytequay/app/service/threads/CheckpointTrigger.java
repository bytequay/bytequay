...
public interface CheckpointTrigger
{
     ...
     *  Implementations decide whether the thread has crossed the
     *  per-thread token threshold and should generate a new segment.
     *  Must return promptly — the call is on the session's event
     *  thread; any heavy lifting (Anthropic call) belongs on a
     *  background executor. */
    void onTurnDone(String threadId);
    ...
    Optional<ThreadCheckpoint> manualGenerate(String threadId);

    /** Last error message a background generation attempt produced
     *  for {@code threadId}, or empty when no error is recorded.
    ...
    Optional<String> lastErrorFor(String threadId);
}
