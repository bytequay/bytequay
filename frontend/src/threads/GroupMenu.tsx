...
type Props = {
  ...
  /** Group IDs the thread currently belongs to. Threads are many-to-many
   *  with groups now, so a single thread can have several entries. */
  ...
  onToggle: (threadId: string, groupId: string, present: boolean) => void | Promise<void>;
};

/**
 * Compact "Pin to groups…" popover for a single thread. Rendered as a
 * `⋯` trigger button that toggles a small absolute-positioned menu.
 * Each row is a checkbox-like toggle — checking adds the thread to that
 ...
 */
export default function GroupMenu({ thread, groups, currentGroupIds, onToggle }: Props) {
  ...
  return (
    <div ref={wrapRef} style={wrapStyle}>
      <button
        ...
        style={triggerStyle}
        title="Pin this thread to a group"
        aria-haspopup="menu"
        aria-expanded={open}
      >
        + Pin
        ...
        >
          {groups.length === 0 ? (
            <div style={emptyStyle}>
              No groups yet. Create one from the Threads rail.
            </div>
          ) : (
            groups.map(g => {
              ...
              return (
                <MenuRow
                  ...
                  label={g.name}
                  active={active}
                  onClick={() => void onToggle(thread.id, g.id, !active)}
                />
              );
            })
          )}
