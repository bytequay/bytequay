...
- the same hardened inspector proves an exact clean tree before and after; and
...
missing toolchain/environment condition with retained bounded evidence. A
...
| `FlowRuntime.adoptChangeSet(claim, fence, repositoryRoot, expectedChangeSetRevisionId)` | fenced Task or ordinary `CI_ROUND` writer operation, including its authenticated check tool | Require the exact source `AgentRun` still `RUNNING`; mechanically inspect the bound worktree outside the database transaction; then revalidate claim, fence, Task epoch/status/selected operation, base pointer, predecessor and expected change-set pointer before appending and atomically advancing `Task.current_head_sha`/pointer. `expectedChangeSetRevisionId` is null only for the first adoption. Exact redelivery returns the same immutable row. `CI_CLEANUP` is rejected because cleanup may adopt only through its exact STOPPED final-state token. |
| `FlowRuntime.prepareNonCleanState(...)` / `handoffStoppedCiRunToCleanup(...)` | stopped CI finalizer | Mint a private token only after exact claim/fence/run/STOPPED proof, then inspect outside the transaction. Consumption revalidates that authority and atomically stores the opaque predecessor result, creates one direct `CI_CLEANUP` operation/ticket, swaps the selected-writer pointer without a null gap, and releases the old lease. The immutable CI seal and `NON_CLEAN_HANDOFF` attempt join the same outer transaction. This mandatory safety successor does not use `reserved_mutation_operation_id`; it has no run or lease until the cleanup dispatcher claims it, and the dirty actual head is not an adopted Task head. |
| `FlowRuntime.prepareCiCleanupAdmission(...)` / `startCiCleanupWriter(...)` | claimed cleanup dispatcher | Require the operation's exact immutable cleanup subject, active Task, selected operation, logical H1/C1, seal, and idle persistent CI session. Re-inspect the full sealed physical H2 state outside the transaction, then consume the private token to mint a cleanup-specific fence whose head is H1 and whose tree/evidence bind the seal, plus one new run in the same session. Generic lease acquisition rejects `CI_CLEANUP`. Exact run/fence redelivery reuses both; recovered never-launched `QUEUED` run/no-fence re-inspects and mints only the new claim-bound fence. A stable mismatch/uninspectable state atomically records `ADMISSION_BLOCKED`, settles the operation/ticket without a body, and installs the mutation barrier. If a recovered queued run already exists, retain it as `CANCELED`, store its deterministic program-owned never-launched `AgentResult`/stop proof, and return the same session to `IDLE`; never delete the run. Movement, timeout, and interruption remain retryable. |
| `FlowRuntime.prepareCiCleanupFinalState(...)` / `finalizeCiCleanup(...)` | stopped cleanup finalizer | First require the exact cleanup run's durable `STOPPED` proof, then mechanically classify the final worktree outside the transaction and return only a private authority-bearing token. Generic clean/non-clean preparation and adoption reject `CI_CLEANUP`; the token is consumed once in the CI owner's outer transaction. A clean state appends the single direct H1/C1 output revision with cleanup source op/run, stores the cleanup `AgentResult`/completion and one `CI_FIX_READY`, settles session/op/ticket, and releases pointer/lease. A second exact non-clean state or stable inspection blocker stores objective attention evidence, settles and releases authority, installs `waiting_mutation_state_ref`, and creates neither a third cleanup nor `CI_FIX_READY`. Transaction failure rolls all owned rows back; exact retry never reruns the body. |
...
| `CiLearningCoordinator.recoverExpiredCiLearning(operationId, generation)` | CI-learning recovery dispatcher | Validate the complete immutable receipt/red-repair/GREEN subject graph. No-attempt recovery redrives the same operation; `RESERVED` also reuses an already-created run/session. These branches need not remain current GREEN because the next begin performs that check. Uncertain `ACTIVATED` quarantines only learner facts. For `STOPPED`, replay the exact sealed completion while the CI owner locks and requires current GREEN for candidate creation, otherwise record `MISSED`, all in one transaction. Generic claim recovery rejects this operation kind. |
...
child, all captured child output is instead replaced with a fixed program
diagnostic before storage or return. Output from profiles without such values
remains bounded but may still contain repository secrets, so policy owners must
choose commands accordingly.
...
or a reserved/directly selected cleanup successor must finish,
...
These are structural transitions, not a bypass around an unresolved mutation:
while `waiting_mutation_state_ref` is present, ordinary commands reject every
status change, including resume, completion, and cancellation. The exact typed
recovery owner must first prove and clear that state.
