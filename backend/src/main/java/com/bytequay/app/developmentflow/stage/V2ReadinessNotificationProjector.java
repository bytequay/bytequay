        ...
        implements ExecutionPorts.MaintenanceWork
{
    ...
    public void maintain(Instant now)
    {
        ...
        for (String stageId : readyStageIds()) {
            transactions.executeWithoutResult(
                    ignored -> notifyIfNewReadyEdge(stageId, nowMs));
        }
    }
    ...
    private void notifyIfNewReadyEdge(String stageId, long nowMs)
    {
        ...
        ReadyNotification ready = jdbc.queryForObject("""
                SELECT marker.workspace_id, marker.trunk_id, marker.task_id,
                       marker.stage_id, marker.ready_stage_version,
                ...
                WHERE marker.stage_id = ? AND marker.state = 'DELIVERED'
                """, (row, ignored) -> new ReadyNotification(
        ...
        requireNonNull(ready, "claimed readiness marker disappeared");
    ...
    private record ReadyNotification(
            String workspaceId,
            String trunkId,
            String taskId,
            String stageId,
